...
// ── Shared calculation logic (exported for inline use in tRPC) ──────────

export async function calculateHealthScore(tenantId: string) {
  const now = new Date();
  ...
  // Base score for having the platform
  contentEngagement += 25;
  ...
  if (revenueMetrics.length > 0) {
    ...
    // Can calculate ROAS (has both spend and revenue)
  ...
  if (adPerformance < 30)
    insights.push(
      "Conecta plataformas de ads para mejorar tu visibilidad"
    );
  if (contentEngagement < 30)
    insights.push(
      "Activa email marketing y WhatsApp para engagement"
    );
...
export const generateHealthScore = task({
  ...
  run: async (payload: { tenantId: string }) => {
    ...
    console.log(
      `Health score generated for tenant ${tenantId}: ${result.overallScore}`
    );

    return { success: true, overallScore: result.overallScore };
  },
});
